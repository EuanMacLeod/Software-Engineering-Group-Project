...
{
    public partial class DatabaseEngineerPdfTaggingPage : Form
    {
        private readonly User user;

        public DatabaseEngineerPdfTaggingPage(User pUser)
        {
            InitializeComponent();
            user = pUser;
        ...
        public void untagButton_clicked(object sender, EventArgs e)
        {
            //checks that only one row is selected from each dgv
            if (dgvSoftwares.SelectedRows.Count != 1)
            {
                MessageBox.Show("Please Select 1 Software");
                return;
            }

            //Checks if the selected software has a PDF Attached
            if (dgvSoftwares.SelectedRows[0].Cells[8].Value.ToString() == "")
            {
            ...
            //asks the user to confirm that the document being tagged to the software is correct
            DialogResult dr = MessageBox.Show(
                "You Are Untagging:\n" + dgvSoftwares.SelectedRows[0].Cells[8].Value + "\nFrom:\n" +
                dgvSoftwares.SelectedRows[0].Cells[2].Value, "confirm", MessageBoxButtons.YesNo,
                MessageBoxIcon.Question);
            if (dr == DialogResult.Yes)
            {
                //todo move selected document from untagged to tagged
                string pdfName = dgvSoftwares.SelectedRows[0].Cells[8].Value.ToString()
                    .Replace(Constants.TAGGED_PDF_FOLDER_PATH, "");

                string taggedPDFPath = Path.Combine(dgvSoftwares.SelectedRows[0].Cells[8].Value.ToString());
                string untaggedPDFPath = Path.Combine(Constants.UNTAGGED_PDF_FOLDER_PATH + pdfName);
                ...
                try
                {
                    ...
                    using (TransactionScope transaction = new TransactionScope())
                    {
                        movePdf(taggedPDFPath, untaggedPDFPath);

                        DBConnections.getInstanceOfDBConnection().updateSoftware(
                            Convert.ToInt32(dgvSoftwares.SelectedRows[0].Cells[0].Value.ToString()), "");
